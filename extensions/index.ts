...
export default function routerExtension(pi: ExtensionAPI) {
	...
	const actions = {
		...
		registerRouterProvider: () => {
			registerRouterProvider(
				pi,
				{
					get isProviderRegistered() { return isProviderRegistered; },
					set isProviderRegistered(v) { isProviderRegistered = v; },
					get currentConfig() { return currentConfig; },
					get currentModelRegistry() { return currentModelRegistry; },
					get lastExtensionContext() { return lastExtensionContext; },
					get selectedProfile() { return selectedProfile; },
					set selectedProfile(v) { selectedProfile = v; },
					get routerEnabled() { return routerEnabled; },
					set routerEnabled(v) { routerEnabled = v; },
					get lastDecision() { return lastDecision; },
					set lastDecision(v) { lastDecision = v; },
					thinkingByProfile,
					pinnedTierByProfile,
					get accumulatedCost() { return accumulatedCost; },
					set accumulatedCost(v) { accumulatedCost = v; },
				},
				{
					persistState,
					recordDebugDecision,
	...
	registerCommands(
		pi,
		{
			...
			thinkingByProfile,
			get lastDecision() { return lastDecision; },
			get lastNonRouterModel() { return lastNonRouterModel; },
			set lastNonRouterModel(v) { lastNonRouterModel = v; },
			get accumulatedCost() { return accumulatedCost; },
			get debugEnabled() { return debugEnabled; },
			set debugEnabled(v) { debugEnabled = v; },
			get widgetEnabled() { return widgetEnabled; },
			set widgetEnabled(v) { widgetEnabled = v; },
			get debugHistory() { return debugHistory; },
		},
		actions
	);
